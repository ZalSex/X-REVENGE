...
class _CreateVipScreenState extends State<CreateVipScreen> {
  ...
  Future<void> _createVip() async {
    ...
    if (username.isEmpty || password.isEmpty) {
      setState(() { _msg = 'Username dan password wajib diisi!'; _msgSuccess = false; });
      return;
    }
    if (password.length < 6) {
      setState(() { _msg = 'Password minimal 6 karakter!'; _msgSuccess = false; });
      return;
    }
    ...
    try {
      ...
      if (res['success'] == true) {
        setState(() {
          _msg = '✓ Akun VIP "$username" berhasil dibuat!';
          _msgSuccess = true;
          _usernameCtrl.clear();
          _passwordCtrl.clear();
        });
        ...
        setState(() { _msg = res['message'] as String? ?? 'Gagal membuat akun VIP'; _msgSuccess = false; });
      }
    } catch (e) {
      setState(() { _msg = 'Error: $e'; _msgSuccess = false; });
    }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_msg.isNotEmpty)
            Container(
              ...
              decoration: BoxDecoration(
                color: _msgSuccess ? Colors.green.withOpacity(0.1) : Colors.red.withOpacity(0.1),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: _msgSuccess ? Colors.green.withOpacity(0.3) : Colors.red.withOpacity(0.3))),
              child: Row(children: [
                Icon(_msgSuccess ? Icons.check_circle_rounded : Icons.error_outline_rounded,
                  color: _msgSuccess ? Colors.green : Colors.red, size: 16),
                const SizedBox(width: 10),
                Expanded(child: Text(_msg, style: TextStyle(
                  fontFamily: 'ShareTechMono', fontSize: 11,
                  color: _msgSuccess ? Colors.green : Colors.red))),
              ]),
            ),
