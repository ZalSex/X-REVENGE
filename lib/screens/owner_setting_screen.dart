...
class _OwnerSettingScreenState extends State<OwnerSettingScreen>
  ...
  void _showDetailDialog(Map<String, dynamic> user) {
    showDialog(
      context: context,
      barrierColor: Colors.black.withOpacity(0.7),
      builder: (_) => _DetailDialog(user: user, accentColor: _gold),
    );
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildHeaderCard(
          ...
          subtitle: 'Reseller Bisa Buat Akun VIP Untuk User Mereka',
          color: _gold,
        ),
        const SizedBox(height: 20),
        _buildLabel('USERNAME'),
        const SizedBox(height: 8),
        _buildTextField(controller: _rsUsernameCtrl, hint: 'Username reseller...', icon: Icons.person_outline_rounded, color: _gold),
        const SizedBox(height: 16),
        _buildLabel('PASSWORD'),
        const SizedBox(height: 8),
        _buildPasswordField(controller: _rsPasswordCtrl, obscure: _rsObscure, onToggle: () => setState(() => _rsObscure = !_rsObscure), color: _gold),
        const SizedBox(height: 20),
        _buildCreateButton(label: 'BUAT AKUN', loading: _rsLoading, onTap: _createReseller, color: _gold),
        const SizedBox(height: 32),
        _buildListHeader(title: 'DAFTAR RESELLER', color: _gold, onRefresh: _loadResellerUsers),
        const SizedBox(height: 12),
        _rsListLoading
          ? Center(child: CircularProgressIndicator(color: _gold.withOpacity(0.7), strokeWidth: 2))
          : _resellerUsers.isEmpty
            ? _buildEmpty(text: 'Belum ada reseller', color: _gold)
            : Column(children: _resellerUsers.map((u) => _buildUserCard(u, color: _gold, role: 'reseller')).toList()),
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildHeaderCard(
          ...
          title: 'BUAT AKUN VIP',
          subtitle: 'Buat Akun VIP Langsung Dari Owner',
          color: _gold,
        ),
        const SizedBox(height: 20),
        _buildLabel('USERNAME'),
        const SizedBox(height: 8),
        _buildTextField(controller: _vipUsernameCtrl, hint: 'Username VIP...', icon: Icons.person_outline_rounded, color: _gold),
        const SizedBox(height: 16),
        _buildLabel('PASSWORD'),
        const SizedBox(height: 8),
        _buildPasswordField(controller: _vipPasswordCtrl, obscure: _vipObscure, onToggle: () => setState(() => _vipObscure = !_vipObscure), color: _gold),
        const SizedBox(height: 20),
        _buildCreateButton(label: 'BUAT AKUN', loading: _vipLoading, onTap: _createVip, color: _gold),
        const SizedBox(height: 32),
        _buildListHeader(title: 'SEMUA AKUN VIP', color: _gold, onRefresh: _loadVipUsers),
        const SizedBox(height: 12),
        _vipListLoading
          ? Center(child: CircularProgressIndicator(color: _gold.withOpacity(0.7), strokeWidth: 2))
          : _vipUsers.isEmpty
            ? _buildEmpty(text: 'Belum ada akun VIP', color: _gold)
            : Column(children: _vipUsers.map((u) => _buildUserCard(u, color: _gold, role: 'vip')).toList()),
        const SizedBox(height: 80),
      ]),
    );
  }
    ...
    return SizedBox(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: loading
              ? LinearGradient(colors: [color.withOpacity(0.4), color.withOpacity(0.3)])
              : LinearGradient(colors: [color, const Color(0xFFFFA500)]),
            borderRadius: BorderRadius.circular(14),
            boxShadow: loading ? [] : [BoxShadow(color: color.withOpacity(0.3), blurRadius: 16)],
          ),
    ...
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: color.withOpacity(0.2), blurRadius: 30, spreadRadius: 2)],
        ),
